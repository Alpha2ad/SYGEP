...
@section('title', 'Liste des Comptable | ')
@section('content')
                            ...
                            <ul class="header-dropdown m-r--5">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        {{-- <li> <a href="{{route('agencecomptable.show', $comptable)}}" class="bg-green text-white"><i class="material-icons">add</i>Nouveau {{ ucfirst(config('multiauth.prefix')) }}</a></li> --}}
                                ...
                                <tbody>
                                    ...
                                    <td> <ul class="header-dropdown m-r--5">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu pull-right">
                                                {{-- <li><a href="{{ route('agenceComptable.show', $comptable->id) }}" class="mr-3 bg-deep-purple text-white"><i class="material-icons">visibility</i>Detail</a></li> --}}
                                                {{-- <li role="separator" class="divider"></li>
                                                <li><a href="{{ route('agencecomptable.edit', $comptable->id) }} " class="mr-3 bg-cyan text-white" ><i class="material-icons">edit</i>Modification</a></li> --}}
                                                <li role="separator" class="divider"></li>
                                            <li>
                                                @if ($comptable->status == true)
                                                <form method="POST" id="activation-{{ $comptable->id }} " action="{{ route('agenceComptable.approuve', [$comptable->id]) }}" style="display: none;">
                                                        @csrf
                                                        @method('PUT')
                                                    </form>
                                                    ...
                                                    <a type="button" class="mr-3 bg-amber text-white" onclick="if(confirm('Est-vous sur de vouloir Désactivé le compte de {{$comptable->name}} des comptables de {{$comptable->agence->name}}???') ){
                                                        ...
                                                        document.getElementById('activation-{{ $comptable->id }} ').submit();
                                                    }else{
                                                        event.preventDefault();
                                                    } "><i class="material-icons" rel="tooltip" title="desactivation de compte">block</i>Désactivé</a>
                                                    @else
                                                    <form method="POST" id="desactivation-form-{{ $comptable->id }} " action="{{ route('agenceComptable.approuve', [$comptable->id]) }}" style="display: none;">
                                                            @csrf
                                                            @method('PUT')
                                                        </form>
                                                        ...
                                                        <a type="button" class="mr-3 bg-teal text-danger" onclick="if(confirm('Est-vous sur de vouloir Activé le compte de {{$comptable->name}} de comptables de {{$comptable->agence->name}}???') ){
                                                            event.preventDefault();
                                                            document.getElementById('desactivation-form-{{ $comptable->id }} ').submit();
                                                        }else{
                                                            event.preventDefault();
                                         ...
                                         {{-- <li role="separator" class="divider"></li>
                                         <li>
                                                <form method="POST" id="delete-form-{{ $comptable->id }} " action="{{ route('agenceComptable.destroy', [$comptable->id]) }}" style="display: none;">
                                                 @csrf
                                                 @method('DELETE')
                                             </form>
