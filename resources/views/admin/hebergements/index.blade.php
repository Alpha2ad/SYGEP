...
<section class="content">
  <div class="container-fluid">
    <div class="block-header">
      ...
      <a class=" btn waves-effect  bg-teal"  href=" {{ route('admin.hebergements.create') }} "><i class="material-icons">add</i><span>Ajouter un hébergement</span></a>
    </div>
              ...
              <div class="body">
               ...
               <td class=""> <ul class="header-dropdown m-r--5 ">
                <li class="dropdown">
                  ...
                  <ul class="dropdown-menu pull-right ">
                    <li class="white"><a href="{{ route('admin.hebergements.show', $hebergement->id) }}" class="btn waves-effect white"><i class="material-icons">visibility</i>Detail</a></li>
<li>
                                                    @if ($hebergement->status == true)
                                                    <form method="POST" id="activation-{{ $hebergement->id }} " action="{{ route('admin.hebergement.approuve', [$hebergement->id]) }}" style="display: none;">
                                                            @csrf
                                                            @method('PUT')
                                                        </form>
                                                        ...
                                                        @else
                                                        <form method="POST" id="desactivation-form-{{ $hebergement->id }} " action="{{ route('admin.hebergement.approuve', [$hebergement->id]) }}" style="display: none;">
                                                                @csrf
                                                                @method('PUT')
                                                            </form>
                    ...
                    <li><a href="{{ route('admin.hebergements.edit', $hebergement->id) }}" class="btn waves-effect white-text"><i class="material-icons">edit</i>Modification</a></li>
                    <li>
                    </li>
                    <li>
                     <form method="post" id="delete-form-{{ $hebergement->id }} " action="{{ route('admin.hebergements.destroy', $hebergement->id) }}" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
