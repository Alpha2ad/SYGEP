  ...
  @section('title', 'Liste des des acounts | ')
  ...
  <section class="content">
    <div class="container-fluid">
      <div class="block-header">
        {{-- <button type="button" class="btn btn-default waves-effect m-r-20" data-toggle="modal" data-target="#formacounts"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;"><i class=" material-icons large bold">add</i> Ajouter</font></font></button> --}}

        {{-- <a class=" btn bg-teal waves-effect "  href=" {{ route('admin.acounts.create') }} "><i class="material-icons">add</i><span>Ajouter un Colis</span></a> --}}
      </div>
      <!-- Exportable Table -->
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <div class="card">
            <div class="header bg-teal card">
              <h2 >
                Liste des acounts
                <span class="badge bg-white">{{ $acounts->count()}}</span>
              </h2>
            ...
            <div class="body">
              <div class="table-responsive">
                <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                  ...
                  <tbody>
                   ...
                   <tr>
                    <td>{{ $key +1 }}</td>
                    <td>{{ $acount->pelerin->agence->name }}</td>
                    <td>{{ $acount->nom }}</td>
                    <td>{{ $acount->paiements->count() }}</td>
              {{-- <td>
             ...
             <td class=""> <ul class="header-dropdown m-r--5 ">
              <li class="dropdown">
                ...
                <ul class="dropdown-menu pull-right ">
                  <li class="white"><a href="{{ route('admin.acounts.show', $acount->id) }}" class="btn waves-effect white"><i class="material-icons">visibility</i>Detail</a></li>
                  <li><a href="{{ route('admin.acounts.edit', $acount->id) }}" class="btn waves-effect white-text"><i class="material-icons">edit</i>Modification</a></li>
                  <li>
                  </li>
                  <li>
                   <form method="post" id="delete-form-{{ $acount->id }} " action="{{ route('admin.acounts.destroy', $acount->id) }}" style="display: none;">
                    @csrf
                    @method('DELETE')
                  </form>
                  ...
                  <a  class="btn  waves-effect white-text" onclick="if(confirm('Est-vous sur de vouloir effectuez cette action??? Cette action est iréversible') ){
                    ...
                    document.getElementById('delete-form-{{ $acount->id }} ').submit();
                  }else{
                    event.preventDefault();
