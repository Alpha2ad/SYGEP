...
<section class="content">
    <div class="container-fluid">
        ...
        <div class="row clearfix">
            ...
            <div class="row clearfix">
                <!-- Task Info -->
                <form method="POST" action=" {{route('admin.comptables.update',[$comptable->id])}}">
                    @csrf
                    @method('patch')
                    <div class="col-xs-12 col-sm-12 col-md-8 col-lg-8">
                        <div class="card">
                            <div class="header bg-teal">
                                <span class="card-title">Modification des informations de L'comptableistrateur</span>
                                <ul class="header-dropdown m-r--5">
                                    <li class="dropdown">
                                        ...
                                        <ul class="dropdown-menu pull-right">
                                            ...
                                            {{-- <li> <a href="{{route('comptable.roles')}}" class="bg-cyan text-white"><i class="material-icons">add</i>Nouveau Role</a></li> --}}
                                        </ul>
                                    </li>
                                </ul>
                            </div>
                            <div class="body">
                                ...
                                <div class="input-group">
                                    ...
                                    <div class="form-line">
                                        <input id="name" type="text" value="{{ $comptable->name }}" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name') }}" placeholder="Nom" required autofocus>
                    ...
                    <div class="input-group">
                        ...
                        <div class="form-line">
                            <input id="email" type="email" value="{{ $comptable->email }}" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" placeholder="Addresse E-Mail" required>
                ...
                <div class="body">
                    ...
                    <select name="agence_id" id="agence_id" class=" {{ $errors->has('agence_id') ? ' is-invalid' : '' }}" data-live-search="true"  >
                        ...
                        <option value="{{ $agence->id }}"
                            @if (in_array($agence->id,$comptable->agence->pluck('id')->toArray()))
                            selected
                            @endif >{{ $agence->name }}
                        </option>
                        @endforeach
                    ...
                    <button class="btn bg-teal waves-effect m-t-15 p-l-10" type="submit"><i class=" material-icons">done_all</i> {{ __('Mettre à jour ') }}</button>
                </div>
            </div>
        </div>
    </form>
